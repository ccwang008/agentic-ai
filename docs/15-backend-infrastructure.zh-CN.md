...
sequenceDiagram
    ...
    C->>API: POST runs，携带目标
    API->>DB: 创建运行记录，状态为 queued
    API->>Q: 将 runId 入队
    API-->>C: 返回 202 Accepted 和 runId
    C->>S: 订阅 runId 对应运行的事件
    W->>Q: 通过 CAS 认领 runId
    W->>DB: 转换为 running
    loop 步骤边界
        W->>DB: 写入检查点
    ...
    W->>DB: 标记为 completed 或 waiting_for_approval
    W->>S: 发出终止事件
```
...
```mermaid
flowchart LR
    A["嵌入式：单进程，<br/>单用户"] --> B["单机，<br/>多进程<br/>网关加工作进程"]
    B --> C["多机：负载均衡器加<br/>无状态服务器加<br/>共享存储"]
    C --> D["控制平面加<br/>工作进程池<br/>调度器是<br/>唯一单例"]
```
