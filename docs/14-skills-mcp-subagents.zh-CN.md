...
```mermaid
flowchart TD
    Q1{"它主要是稳定的<br/>流程性知识吗？"}
    Q1 -- 是 --> Skill["编写技能"]
    Q1 -- 否 --> Q2{"它需要代码、状态、<br/>凭证或外部服务吗？"}
    Q2 -- 是 --> MCP["编写或使用 MCP 服务器"]
    Q2 -- 否 --> Q3{"它需要自己的<br/>多步推理和<br/>上下文吗？"}
    Q3 -- 是 --> Subagent["创建子智能体"]
    Q3 -- 否 --> Local["保留为内置工具<br/>或放在父智能体提示词中"]
```
