...
```mermaid
flowchart TD
    Code["代码 + 技能 + 提示词"] --> Build["构建流水线"]
    Build --> Pkg["软件包：二进制、容器、npm 或 pypi"]
    Pkg --> Deploy["部署：环境配置、密钥、迁移"]
    Deploy --> Run["运行：健康状态、就绪状态、指标（第 16 章）"]
    Run --> Inc{"发生事故？"}
    Inc -- 否 --> Run
    Inc -- 是 --> Runbook["运行手册：检测、遏制、调查、恢复、学习（第 18 章）"]
    Runbook --> PM["事故复盘"]
    PM --> Code
```
...
```mermaid
flowchart LR
    S1["阶段 1<br/>一名开发者，一台机器<br/>cron 作业，本地日志"] --> S2["阶段 2<br/>一个团队，一个运行节点<br/>Docker，共享 NFS，Slack 告警"]
    S2 --> S3["阶段 3<br/>多实例<br/>Postgres，预算，PagerDuty"]
    S3 --> S4["阶段 4<br/>由 SRE 管理<br/>多区域，可观测性栈，金丝雀发布"]
```
