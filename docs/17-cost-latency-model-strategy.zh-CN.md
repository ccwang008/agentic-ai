...
```mermaid
flowchart TD
    Task["传入的步骤或调用"] --> L1{"任务到配置"}
    L1 -- 规划器 --> Plan["规划智能体使用 fast"]
    L1 -- 构建器 --> Build["构建智能体使用 deep"]
    L1 -- 审查器 --> Rev["审查智能体使用 fast"]
    L1 -- 压缩器 --> Comp["压缩使用辅助 fast"]
    Plan --> L2{"配置到服务商"}
    Build --> L2
    Rev --> L2
    Comp --> L2
...
```mermaid
flowchart LR
    Traces["来自第 16 章的追踪语料库"] --> Old["当前配置"]
    Traces --> New["候选配置"]
    Old --> Compare["比较任务结果"]
    New --> Compare
    Compare --> Pass{"没有严重回归？"}
